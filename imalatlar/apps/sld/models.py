...
class SLD(WorkOrderAbstractModel):
    ...
    DOOR_TYPES = (
        ('th_standart', 'STANDART TEK HAREKETLI'),
        ('ts_th_standart', 'STANDART TEK SBT. + TEK HRK.'),
        ('ih_standart', 'STANDART IKI HAREKETLI'),
        ('is_ih_standart', 'STANDART IKI SBT. + IKI HRK.'),
        ('th_cam', 'CAM - TEK HAREKETLI'),
        ('ts_th_cam', 'CAM - TEK SBT. + TEK HRK.'),
        ('ih_cam', 'CAM - IKI HAREKETLI'),
        ('is_ih_cam', 'CAM - IKI SBT. + IKI HRK.'),
        ('ih_teleskop', 'TELESKOPIK - IKI HAREKETLI'),
        # ('ih_is_teleskop', 'TELESKOPİK - İKİ HRK. + İKİ SBT.'), # iki hareketli tek sabit olarak duzeltildi.
        ('ih_ts_teleskop', 'TELESKOPIK - IKI HRK. + TEK SBT.'),
        ('dh_teleskop', 'TELESKOPIK - DORT HAREKETLI'),
        ('dh_is_teleskop', 'TELESKOPIK - DORT HRK. + IKI SBT.'),
    )
    ...
    opening_direction = models.CharField('Acilis Yonu', max_length=3,
                                         choices=(('sag', 'SAG'), ('sol', 'SOL')), null=True, blank=True)
    ustluk = models.BooleanField('UstlUk Var Mi?', default=False)
    door_type = models.CharField('Kapi Tipi', choices=DOOR_TYPES, max_length=30)
    pass_width = models.PositiveSmallIntegerField('Gecis Genisligi')
    pass_height = models.PositiveSmallIntegerField('Gecis Yüksekligi')
    total_width = models.PositiveSmallIntegerField('Toplam Genislik', null=True, blank=True)
    total_height = models.PositiveSmallIntegerField('Toplam Yükseklik', null=True, blank=True)
    mechanism_width = models.PositiveSmallIntegerField('Mekanizma Genisligi', null=True, blank=True)
    ...
    def __str__(self):
        return f'{self.crm} - {self.company_name} - {self.door_type}' # crm no ve firma isimleri database ve admin panelinde gozukur.

    def get_absolute_url(self):
        return reverse('sld:detail', kwargs={'id': self.id}) # nesnelerimizin adreslemesini yapariz.
